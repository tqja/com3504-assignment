  ...
  event.waitUntil(
    (async () => {
      ...
      try {
        ...
        await cache.addAll(
          requests.concat(uploads).concat(javascripts).concat(stylesheets),
        );
  ...
  if (event.tag === "sync-observations") {
    event.waitUntil(
      openNSyncObservationsIDB()
        ...
        .then((observations) => {
          return Promise.all(
            observations.map((observation) => syncNewObservation(observation)),
          );
        })
        .catch((error) => {
    ...
    .then(async (savedObservation) => {
      ...
      return Promise.all([
        ...
        openNSyncObservationsIDB().then((db) =>
          deleteNSyncObservation(db, syncID),
        ),
      ]);
    })
    .catch((error) => {
